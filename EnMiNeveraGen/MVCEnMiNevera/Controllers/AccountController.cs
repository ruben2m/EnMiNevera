            ...
            if (hasLocalAccount)
            {
                if (ModelState.IsValid)
                {
                    ...
                    try
                    {
                        ...
                        UsuarioCEN cen = new UsuarioCEN();
                        UsuarioEN en = cen.get_IUsuarioCAD().ReadOIDDefault(WebSecurity.GetUserId(User.Identity.Name));
                        cen.Modify(en.Id, en.Nombre, en.Contrasena, en.Email, en.Apellidos, en.Nick, en.Foto, en.Biografia, en.FechaNacim, en.Baneado, en.Rol);
                    }
                    catch (Exception)
                    {
